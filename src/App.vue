    ...
    <div class="container">
      ...
      <template v-if="tickers.length">
        ...
        <div style="display: flex; justify-content: end;">
          <div>
            Filter: 
            <input v-model="filter" style="text-transform: uppercase;"/>
          </div>
        </div>
        <hr class="w-full border-t border-gray-600 my-4" />
...
export default {
  ...
  computed: {
    ...
    filteredTickers() {
      return this.tickers.filter(ticker => ticker.name.toUpperCase().includes(this.filter.toUpperCase()));
    },
  ...
  methods: {
    ...
    add(ticker) {
      ...
      if (this.tickers.map(t => t.name).includes(currentTicker.name)) {
        return alert('This ticker already exist!')
      }
